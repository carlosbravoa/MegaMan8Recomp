...
| B10 | **Whole-game coverage** | The starter pack behind everything so far is boot → intro stage. ⬜ Dumps of every stage / menu / boss / ending (scripted from savestates + manual play with `texture_dump` armed, `pairs.tsv` accumulated), `coverage --missing`, per-stage sheets; palette-cycling recolours (buster charge flash, energy/water cycles) get their `<tex>-<pal>` variants from these dumps. | medium, mechanical |
| B11 | **SW ↔ GL parity tool** | ⬜ Same shape as `tools/video_filter_check.py`: identical state on both renderers with a pack, assert ≤ 1 LSB per texel; today GL is verified only by eye + one windowed diff. | small |
| B12 | **Names from Track A** | ⬜ Texel ids are hashes; artists want `megaman_run_03.png`. Cross-reference dumps with the PAC sprite/tile tables (A3/A3b) → `names.tsv`, `texpack.py` sheets grouped by PAC/section/frame. | medium |
| B13 | **Odds and ends** | ⬜ Bilinear sampling of pack images (matters once art is finer than N× nearest); 8bpp textures (MM8 is 4bpp so far); shaded-textured triangles (3D titles); document that a hand-made `<tex>-<pal>.png` without `.clut` is exact-palette-only. | small each |

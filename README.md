...
| **HD movies** (replace the six MDEC cutscenes with any video at any resolution; the STR still streams and keeps its own audio, so sync is automatic) | `PSX_FMV_DUMP=$PWD/build-debug/fmvdump bash tools/run_mm8.sh` once and let the movies play (native frames land as `fmvdump/<MOVIE>/NNNNN.png` — the exact frame numbering); then per movie `python3 psxrecomp/tools/fmv_pack.py from-video my_ROCK8_0.mp4 game-assets/movies/pack/ROCK8_0 --frames <count> [--jpg]` (any fps/length, ffmpeg resamples to the count), `fmv_pack.py check`, tick **HD movies** in the launcher. `psxrecomp/docs/FMV_PACKS.md`. |

...
| **Start at any stage** (testing aid — bookmarks are savestates, valid only for the same disc dump + BIOS they were taken with; the row appears only when `saves/bookmarks/` has files) | Take a savestate where you want to start (slot save in game), `bash tools/bookmark.sh add <slot> "04 Grenade Man"`; the launcher's SYSTEM card gets a **Start at** row listing `saves/bookmarks/*.pst`, and the game resumes there right after boot (in-game launcher: loads immediately). Also `--start-state file.pst` / `PSX_START_STATE`. `psxrecomp/docs/BOOKMARKS.md`. Bookmark files hold game memory — share privately, never commit. |
| **HD movies** (replace the six MDEC cutscenes with any video at any resolution; the STR still streams and keeps its own audio, so sync is automatic) | `python3 psxrecomp/tools/fmv_pack.py export-str game-assets/disc/cdrom/MOVIE/*.STR --out movies/` gives you the six movies as MP4 (video + audio, one frame per STR frame — feed those to your upscaler); then per movie `python3 psxrecomp/tools/fmv_pack.py from-video my_ROCK8_0.mp4 game-assets/movies/pack/ROCK8_0 --frames <count> --jpg` (same frame count → copied 1:1, else resampled), `fmv_pack.py align <STR> <video> --write game-assets/movies/pack/ROCK8_0` if the upscaler trimmed a lead-in, tick **HD movies** in the launcher. `psxrecomp/docs/FMV_PACKS.md`. |
| **HD textures** (upscaled / redrawn art at draw time, both renderers) | `bash tools/texdump_sweep.sh` builds the whole-game starter pack (every background tile straight from the PACs + Mega Man's sprites + headless play dumps of title/menus/stages/savestates and of every bookmark in `saves/bookmarks/`, merged) at `game-assets/textures/pack`; to add what only real play reaches, run with `PSX_TEXTURE_DUMP=D` while you play and `python3 psxrecomp/tools/texpack.py merge build-debug/texdumps/all D` → `starter`. `python3 psxrecomp/tools/texpack.py export game-assets/textures/pack work/ --group PLAYER/` gives you human-named working copies (`work/PLAYER/strip057_cell07.png`, from `names.tsv`), `import work/ game-assets/textures/pack` puts them back. Repaint the PNGs (keep the `.clut` sidecars: they let palette fades / flashes dim your art like the original), set Display → Supersampling ≥ 2 and tick **HD textures** in the launcher. `{"cmd":"texture_pack","op":"stats"}` / `"usage"` tell you which files were actually drawn. `psxrecomp/docs/TEXTURE_PACKS.md`. |

...
| **HD movies** (replace the six MDEC cutscenes with any video at any resolution; the STR still streams and keeps its own audio, so sync is automatic) | `python3 psxrecomp/tools/fmv_pack.py export-str game-assets/disc/cdrom/MOVIE/*.STR --out movies/` gives you the six movies as MP4 (video + audio, one frame per STR frame — feed those to your upscaler); then per movie `python3 psxrecomp/tools/fmv_pack.py from-video my_ROCK8_0.mp4 game-assets/movies/pack/ROCK8_0 --frames <count> [--jpg]` (any fps/length, ffmpeg resamples to the count), `fmv_pack.py check`, tick **HD movies** in the launcher. `psxrecomp/docs/FMV_PACKS.md`. |
